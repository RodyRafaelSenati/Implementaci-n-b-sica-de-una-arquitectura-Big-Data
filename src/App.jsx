...
const DashboardContent = () => {
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 text-slate-100 selection:bg-indigo-500 selection:text-white">
      ...
      {/* 2. Área de Contenido Principal (Full Screen Fluid) */}
      <div className="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        {/* Contenedor Fluido con Adaptación Total a 1080p / 2K / 4K */}
        <main className="flex-1 w-full max-w-[1920px] mx-auto px-4 sm:px-6 2xl:px-8 py-6 space-y-6">
          
          {/* Banner de Sincronización Big Data */}
          {loading && (
            <div className="p-4 rounded-2xl bg-indigo-950/60 border border-indigo-500/40 text-indigo-200 text-xs flex items-center justify-between animate-pulse">
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin"></div>
                <span className="font-semibold">Sincronizando con el clúster HDFS y MongoDB NoSQL...</span>
              </div>
